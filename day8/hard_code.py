...
# new condition: antinode can occur as many times as the distance between antennas
    ...
    for frequency in frequencies:
        for i in range(len(frequencies[frequency])):
            for j in range(i+1, len(frequencies[frequency])):
                ...
                # start on opposite antenna to account for overlapping antinodes
                # left side
                k = 1
                while x2 - k*dx >= 0 and y2 - k*dy >= 0:
                    antinodes.add((x2 - k*dx, y2 - k*dy))
                    k += 1
                # right side
                k = 1
                while x1 + k*dx < len(attenna_map[0]) and y1 + k*dy < len(attenna_map):
